...
// Note: Balise creation:
// $.BaliseName({"attribute1":"value1",...},textInsideTag)
function logIvo(msg) {
	console.info(msg);
}
...
var GSN = {
	...
	,
	vsName : new Array(),
	protect : new Array() // characterizes the above, if it is protected or
	// not
	,
	...
	init : function(data) {
		...
		if ($(document).attr("title") == "GSN") {
			...
			$("#gsn-author").empty().append(
					$(gsn).attr("author") + " (" + $(gsn).attr("email") + ")");
		}
		...
		// Make a matrix of the array of the sensor with the group name in the
		// second column

		// Test example
		// GSN.vsName.push("genepi_meteo_10_replay",
		// "genepi_meteo_11_replay","genepi_meteo_12_replay","genepi_meteo_13_replay","genepi_meteo_15_replay","genepi_meteo_16_replay","genepi_meteo_18_replay","genepi_meteo_2_replay","genepi_meteo_3_replay","genepi_meteo_4_replay","genepi_meteo_6_replay","genepi_meteo_7_replay");
		GSN.vsName = GSN.util.regroupByUnderscore(GSN.vsName);
		GSN.protect.sort();
		for ( var i = 0; i < GSN.vsName.length; ++i) {
			logIvo(GSN.vsName[i] + "--" + GSN.protect[i][0] + "--"
					+ GSN.protect[i][1]);
		}
		...
		for ( var i = 0; i < vsName.length; ++i) {
			if (vsName[i][1] != previousCategoryName
					&& vsName[i][1] != "others") {
				...
				if (GSN.context == "data") {
					$("#vsmenu").append(
							$.A({
								"class" : "rubric",
								"href" : "javascript:GSN.util.toggle($(\"."
										+ vsName[i][1] + " span\"));",
			...
			if (vsName[i][1] != "others") {
				...
				GSN.numSensorAssociatedWithCategory.setItem(vsName[i][1],
						GSN.numSensorAssociatedWithCategory
								.getItem(vsName[i][1]) + 1);
			}
		}// End for
		...
		for ( var i = 0; i < vsName.length; ++i) {
			if (vsName[i][1] == "others")
				othersRubricSensorPresent = true;
		}
					...
					if (GSN.context == "data") {
						...
						$("#vsmenu").append(
								$.DIV({
									...
									"class" : "sensorName",
									"href" : "javascript:GSN.removeVS('"
											+ vsName[i][0] + "');",
									"id" : "menu-" + vsName[i][0] + ""
								}, "X"), $.A({
									"class" : "sensorName",
									"href" : "javascript:GSN.menu('"
											+ vsName[i][0] + "');",
									"id" : "menu-" + vsName[i][0] + ""
								}, vsName[i][0] + GSN.protect[i][1])));
					GSN.numSensorAssociatedWithCategory.setItem(vsName[i][1],
							GSN.numSensorAssociatedWithCategory
									.getItem(vsName[i][1]) + 1); // added May
					// 2013
				}
			}
		}
					...
					.droppable(
							{
								accept : ".ui-draggable",
								drop : function(ev, ui) {
									var sensorDroppedName = $(
											ui.draggable.element).text();

									if (!GSN
											.isAlreadyInSelectedSensorArray(sensorDroppedName)) {
										if (sensorDroppedName.substr(0, 9) == "  Group: ") {
											// If a group has been dropped
											GSN
													.AddSensorGroupToTheDraggableArea($(
															ui.draggable.element)
															.text().substr(9));
										} else if (sensorDroppedName == "  Others") {
											// If the group 'others' has been
											// dropped
											GSN
													.AddSensorGroupToTheDraggableArea("others");
										} else {
											// If a sensor has been dropped
											GSN.AddSensorToTheDraggableArea(
													sensorDroppedName,
													GSN.selectedSensors.length);
										}
									}
	...
	isAlreadyInSelectedSensorArray : function(sensorName) {
		// Look in the selected sensor array to see if the selected sensor don't
		// already belongs to this former
	...
	removeVS : function(vsName) {
		$.ajax({
			type : "GET",
			url : "/gsn?REQUEST=120",
			data:{'VSFileName':vsName},
			success : GSN.reloadPage(),
	...
	AddSensorToTheDraggableArea : function(sensorDroppedName, colorID) {
		...
		$("#inDraggableArea-" + sensorDroppedName).css("border-left",
				"10px solid #FFA84C");

		// Add a link do delete the sensor from the draggable area
		$("#inDraggableArea-" + sensorDroppedName).append(
				"<a href=\"javascript:GSN.removeFromDraggableArea('"
						+ sensorDroppedName
						+ "')\"><img src=\"../img/button_cancel.png\"/></a>");
		$("#inDraggableArea-" + sensorDroppedName).append(
				"&nbsp;&nbsp;" + sensorDroppedName + "");
			...
			$("#inDraggableArea-" + sensorDroppedName)
					.append(
							'<div class="colorpicker" id="colorpicker-'
									+ sensorDroppedName
									+ '" style="display:none;position:relative;left:80px;"></div>');
			$("#inDraggableArea-" + sensorDroppedName)
					.append(
							'<input type="text" maxlength="7" size="7" onclick="if($(\'#colorpicker-'
									+ sensorDroppedName
									+ '\').css(\'display\') == \'none\'){$(\'.colorpicker\').hide(\'slow\');$(\'#colorpicker-'
									+ sensorDroppedName
									+ '\').show(\'slow\');$(\'#colorpickerButton-'
									+ sensorDroppedName
									+ '\').show();}" id="hexcode-'
									+ sensorDroppedName
									+ '" name="hexcode-'
									+ sensorDroppedName
									+ '" value="#'
									+ DEFAULT_COLOR_CODE[colorID
											% DEFAULT_COLOR_CODE.length]
									+ '" style="float:right;position:relative;top:-20px;width:55px;border-width:1px;"/>');
			$("#inDraggableArea-" + sensorDroppedName)
					.append(
							'<input type="button" class="colorpicker" id="colorpickerButton-'
									+ sensorDroppedName
									+ '" onclick="$(\'.colorpicker\').hide(\'slow\');" value="Ok" style="display:none;float:right;position:relative;top:-22px;left:-5px;"/>');

			$('#colorpicker-' + sensorDroppedName).farbtastic(
					'#hexcode-' + sensorDroppedName);
		} catch (err) {
			$("#colorpicker-" + sensorDroppedName).remove();
			$("#hexcode-" + sensorDroppedName).remove();
			$("#inDraggableArea-" + sensorDroppedName)
					.append(
							'<input type="text" maxlength="7" size="7" id="hexcode-'
									+ sensorDroppedName
									+ '" name="hexcode-'
									+ sensorDroppedName
									+ '" onchange="$(this).css(\'background-color\',$(this).val())" value="#'
									+ DEFAULT_COLOR_CODE[colorID
											% DEFAULT_COLOR_CODE.length]
									+ '" style="float:right;position:relative;top:-20px;width:55px;border-width:1px;background-color:#'
									+ DEFAULT_COLOR_CODE[colorID
											% DEFAULT_COLOR_CODE.length]
									+ ';"/>');
		...
		newSize = parseInt($("#dropArea").css('height').substr(0,
				stringLength - 2))
				+ 30 + 'px';
		...
		newSize = parseInt($("#dropAreaMask").css('height').substr(0,
				stringLength - 2))
				+ 30 + 'px';
	...
	/**
	 * Add a sensor group to the drop Area add the sensors to the selected
	 * sensor array
	 */
	,
	AddSensorGroupToTheDraggableArea : function(groupName) {
		if (confirm("Are you sure that you want to add the "
				+ GSN.numSensorAssociatedWithCategory.getItem(groupName)
				+ " sensors of the '" + groupName + "' group?")) {
			var nbSelectedSensor = GSN.selectedSensors.length;
			var nbSensorToAdd = GSN.numSensorAssociatedWithCategory
					.getItem(groupName);
			var nbSensor = GSN.vsName.length;

			for ( var i = 0; i < nbSensor; ++i) {
				if (GSN.vsName[i][1] == groupName) {
					// If this sensor is already in the selected sensor array we
					// remove it to have after a nive grouping inside the drop
					// area
	...
	/**
	 * Remove From drop Area the sensor and remove the sensor from the selected
	 * sensor array
	 */
	,
	removeFromDraggableArea : function(sensorName) {
		...
		for ( var i = 0; i < GSN.vsName.length; ++i) {
			if (GSN.vsName[i][0] == sensorName)
				nameCategory = GSN.vsName[i][1];
		}

		// alert(GSN.selectedSensors+" "+nameCategory+" "+$("#vsmenu
		// #menu-rubric-"+nameCategory).length);
	...
	updateall : function(num, showall) {
		...
		//if (typeof num == "number" && num != GSN.updatenb)
		//	return;
		GSN.updatenb++;
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (firstload && GSN.context == "home") {
					for ( var i = 0; i < 10; ++i) {
						var n = $($("virtual-sensor", data).get(i))
								.attr("name");
						if (n != null)
							GSN.vsbox.add(n);
					}
				...
				// next refresh
				if ($("#refreshall_timeout").attr("value") > 0)
					setTimeout("GSN.updateall(" + GSN.updatenb + ")", $(
							"#refreshall_timeout").attr("value"));
	...
	addandupdate : function(vsName) {
		...
		$.ajax({
			...
			url : "/gsn?name=" + vsName,
			success : function(data) {
				$("virtual-sensor[@name=" + vsName + "]", data).each(
						function() {
							GSN.vsbox.update(this);
						});
			}
	...
	vsbox : {
		...
		add : function(vsName) {
			...
			if ($(this.container).find("." + vsdiv).size() != 0)
			...
			$(this.container).append(
					$.DIV({
						...
						"href" : "javascript:GSN.vsbox.toggle('" + vsName
								+ "','dynamic');",
			...
			$(this.container)
					.find("." + vsdiv + " dl.upload")
					.html(
							'<form target="webupload" enctype="multipart/form-data" method="post" action="/upload"><input value="'
									+ vsName
									+ '" name="vsname" type="hidden"><select name="cmd" class="cmd"></select><dl class="input"></dl><input value="upload" type="submit"><p>* compulsary fields.</p></form>');
			$(this.container).find("." + vsdiv + " select.cmd").bind("change",
					function(event) {
		...
		update : function(vs) {
			...
			if ($(dynamic).children().size() == 0
					&& $(static_).children().size() == 0) {
				...
				$("field", vs)
						.each(
								function() {
									...
									if (name == "timed") {
										// if (value != "") value =
										// GSN.util.printDate(value);
										$(vsd).find("span.timed").empty()
												.append(value);
										return;
									}
									...
									if (cat == "input") {
										...
										if ($(this).attr("description") != null)
											s += ' <img src="style/help_icon.gif" alt="" title="'
													+ $(this).attr(
															"description")
													+ '"/>';
										$(struct).append(
									...
									if (cat == null) {
										if (value == "") {
											...
											value = '<embed type="image/svg+xml" width="400" height="400" src="'
													+ value
											...
											value = '<a href="'
													+ value
													+ '">download <img src="style/download_arrow.gif" alt="" /></a>';
										}
									} else if (cat == "input") {
										if (last_cmd != cmd) {
											if (last_cmd != null)
												hiddenclass = ' hidden';
											$("select.cmd", vsd).append(
													$.OPTION({}, cmd));
											last_cmd = cmd;
										}
										var comp = '';
										...
										if (type.split(":")[0]
												.indexOf("binary") != -1) {
											value = '<input type="file" name="'
													+ cmd + ";" + name + '"/>';
										} else if (type.split(":")[0]
												.indexOf("select") != -1) {
											var options = type.split(":")[1]
													.split("|");
											value = '<select name="' + cmd
													+ ";" + name + '">';
										...
										if ($(this).attr("description") != null)
											value += ' <img src="style/help_icon.gif" alt="" title="'
													+ $(this).attr(
															"description")
													+ '"/>';
				...
				if ($(vs).attr("description") != "") {
					...
					if (GSN.protect[i][1] == " ") {
						...
						$("dl.description", vsd).empty().append('<p> Bla')
								.append($.DD({}, value2)).append('</p>');
						logIvo("Iter = " + $(vs).attr("description"));
					} else {
						// $("dl.description",
						// vsd).empty().append($.DD({},$(vs).attr("description")));
						value2 = $(vs).attr("description");
						...
						$("dl.description", vsd).empty().append(res).append(
								'<dl> <dt> Visibility: </dt> </dl>').append(
								$.DD({}, dummy));
						// $("dl.description", vsd).empty().append('<dl> <dt>
						// Bla: </dt> <dd>').append($.DD({},
						// value2)).append('</dd> </dl>');
					}
	...
	map : {
		...
		addMarker : function(vsName, lat, lon) {
			...
			if (mapProvider == "microsoft") {
				...
				marker
						.setInfoBubble("Show/Hide Information: <a style='text-decoration:underline;color:blue;' href='javascript:GSN.menu(\""
								+ vsName
								+ "\");if (GSN.context==\"fullmap\")GSN.vsbox.bringToFront(\""
								+ vsName + "\");'>" + vsName + "</a>");
			...
			if (mapProvider == "google") {
				marker.setIcon("./img/green_marker.png");
				marker
						.setInfoBubble("<script>GSN.menu(\""
								+ vsName
								+ "\");if (GSN.context=='fullmap')GSN.vsbox.bringToFront(\""
								+ vsName + "\");</script>Selected Sensor: "
								+ vsName);
				GSN.map.markers.push(marker);
			}
			...
			if (GSN.context == "fullmap") {
				...
				$(vs).wrap(
						"<a href=\"javascript:GSN.menu('" + $(vs).text()
								+ "');\"></a>");
			}
		}
		...
		/**
		 * Highlight a marker Stop it if called with null name
		 */
		,
			...
			if (vsName != null) {
				for (x in GSN.map.markers) {
					...
					if (m.getAttribute("vsname") == vsName) {
						...
						map.setCenter(new LatLonPoint(m.location.lat,
								m.location.lon))
						return;
					}
				}
	...
	data : {
		...
		/**
		 * Initialisation of the Data part (called as soon as a user selected
		 * sensors and click on the button)
		 */
		init : function() {
			GSN.data.fields.splice(0);
			GSN.data.fields_type.splice(0);

			$("#step1Container .data").hide("slow").prev().unbind('click')
					.click(function() {
			...
			for ( var i = 1; i < GSN.selectedSensors.length; ++i)
				if (GSN.selectedSensors[i - 1] == GSN.selectedSensors[i])
					GSN.selectedSensors.splice(i, 1);
			...
			$("#formular")
					.append(
							"<input type=\"hidden\" name=\"numberSelectedSensor\" id=\"numberSelectedSensor\" value=\""
									+ GSN.selectedSensors.length + "\">");
			for ( var i = 0; i < GSN.selectedSensors.length; ++i)
				// TODO $("#formular").append("<input type=\"hidden\"
				// name=\"vsName"+i+"\" id=\"vsName"+i+"\"
				// value=\""+GSN.selectedSensors[i]+"\">");
				$("#formular").append(
						"<input type=\"hidden\" name=\"vsname\" id=\"vsName"
								+ i + "\" value=\"" + GSN.selectedSensors[i]
								+ "\">");

			$("#step2Container")
					.append(
							"<div class=\"step\">Step 2/5 : Selection of the Fields</div>");
			$("#step2Container").append(
					"<div class=\"data\" id=\"fields\"></div>");
			$('#fields').append('<br />');
			$('#fields').append('<span>Aggregation</span>');
			$('#fields')
					.append(
							'<select id="agg_function" name="agg_function"><option value="-1">No Aggregation</option><option value="avg">AVG</option><option value="max">MAX</option><option value="min">MIN</option></select>');
			$('#fields')
					.append(
							'<input disabled="disabled" id="agg_period" name="agg_period" size="5" value="2" type="text" />');
			$('#fields')
					.append(
							'<select disabled="disabled" id="agg_unit" name="agg_unit"><option value="3600000">Hours</option><option value="60000">Minutes</option><option value="1000">Seconds</option><option value="1">Milli Seconds</option></select>');
			$('#fields').append('<br /><br />');
			...
			// $("#fields").append("<input type=\"radio\" id=\"commonReq\"
			// name=\"commonReq\"
			// onClick=\"javascript:GSN.data.radio=false;GSN.data.init();\"
			// />Common request ");
			// $("#fields").append("<input type=\"radio\" id=\"aggregReq\"
			// name=\"aggregReq\"
			// onClick=\"javascript:GSN.data.radio=true;GSN.data.init();\"
			...
			for ( var i = 0; i < GSN.selectedSensors.length; ++i) {
				$.ajax({
					...
					success : function(msg) {
						$("virtual-sensor field", msg)
								.each(
										function() {
											if ($(this).attr("type").substr(0,
													3) != "bin") {
												GSN.data.fields.push($(this)
														.attr("name"));
												GSN.data.fields_type.push($(
														this).attr("type"));
											}
										});
		...
		selectCommonFieldAndDisplay : function() {
			...
			for ( var i = 0; i < GSN.data.fields.length; i++) {
				if (GSN.data.radio) {
					...
					// ($(this).attr("type") == "long") || ($(this).attr("type")
					// == "double")) {
					$("#separation")
							.append(
									"<div id='"
											+ GSN.data.fields[i]
											+ "'><input type=\"checkbox\" name=\"fields\" class=\"field\" value=\""
											+ GSN.data.fields[i]
											+ "\" onClick=\"javascript:GSN.data.aggregateSelect('"
											+ GSN.data.fields[i]
											+ "',this.checked)\">"
											+ GSN.data.fields[i].prettyString()
											+ " </div>");
					// }
				} else {
					$("#separation").append(
				...
				if (GSN.data.radio) {
					...
					$("#groupByContainer")
							.append(
									"<br/>Group by : <select name=\"aggregateGB\" id=\"aggregateGB\" size=\"1\" onChange=\"javascript:GSN.data.groupBy(this.value)\"></select><br/>");
					for ( var j = 0; j < GSN.data.fields.length; j++) {
						$("#aggregateGB").append(
								"<option value=\"" + GSN.data.fields[j] + "\">"
										+ GSN.data.fields[j].prettyString()
										+ "</option>");
					}
					$("#aggregateGB").append(
					...
					$("#separation")
							.append(
									"<span id=\"checkAllContainer\"><br/><input type=\"checkbox\" id=\"all\" name=\"all\" onClick=\"javascript:GSN.data.checkAllFields(this.checked)\">Check all<br/></span>");
				}
			}

			GSN.data.appendNextStepButton("separation",
					"if(GSN.data.atLeastOneFieldSelected())GSN.data.nbDatas()");
		},
		...
		/**
		 * Append a button in "idInWichAppend" with an action: "onclick" an
		 * "idButton" and a "value" (configurable)
		 */
		appendNextStepButton : function(idInWichAppend, onclick, idButton,
				value) {
			if (idButton == null)
				idButton = "";
			if (value == null)
			...
			$("#" + idInWichAppend + "")
					.append(
							'<br/><div class="nextStepButton"><input type="button" id="'
									+ idButton
									+ '" value="'
									+ value
									+ '" onClick="'
									+ onclick
									+ ';"/>'
									+ "&nbsp&nbsp&nbsp<input type=\"button\" id=\"resetButton\" value=\"Reset\" onClick=\"window.location='./data.html';\"/></div>");
		},
		...
		groupBy : function(option) {
			if (option == "timed") {
				$("#aggregateGB")
						.after(
								"<input type=\"text\" name=\"gbdelta\" id=\"gbdelta\" size=\"5\"><select name=\"gbdeltameasure\" id=\"gbdeltameasure\" size=\"1\"></select>");
				$("#gbdeltameasure").append(
				...
				$("#gbdeltameasure")
						.append("<option value=\"h\">Hour</option>");
		...
		nbDatas : function() {
			$(".nextStepButton").remove();
			$("#step2Container .data").hide("slow").prev().unbind('click')
					.click(function() {
						$(this).next().toggle("slow");
					});
			...
			$("#step3Container")
					.append(
							"<div class=\"step\">Step 3/5 : Selection of the Data Range</div>");
			$("#step3Container").append(
					"<div class=\"data\" id=\"nbDatas\"></div>");
			$("#nbDatas")
					.append(
							'<br /><select id="nb_selection" name="nb_selection"><option value="ALL">All Data</option><option value="SPECIFIED" selected="selected">Only</option></select>');
			$("#nbDatas")
					.append(
							'<input id="nb_value" name="nb_value" size="3" value="10" type="text"><span> Values</span><br />');
			$('#nb_selection').change(function() {
				var val = $(this).val();
				$('#nb_value').attr('disabled', (val == 'ALL'));
			});
			// $("#nbDatas").append("<input type=\"radio\" name=\"nbdatas\"
			// id=\"allDatas\" value=\"\"> All data<br/>");
			// $("#nbDatas").append("<input type=\"radio\" name=\"nbdatas\"
			// id=\"someDatas\" value=\"\" checked
			// onclick=\"$('#nbOfDatas').focus()\"> Last <input
			// onclick=\"$('#someDatas').attr('checked', 'checked');\"
			// type=\"text\" name=\"nb\" value=\"10\" id=\"nbOfDatas\"
			// size=\"4\"/> values<br/>");
			GSN.data.appendNextStepButton("nbDatas",
		...
		addCriteria : function(newStep) {
			if (newStep) {
				$(".nextStepButton").remove();
				$("#step3Container .data").hide("slow").prev().unbind('click')
						.click(function() {
							$(this).next().toggle("slow");
						});
				$("#step3Container .data :enabled")
						.attr("disabled", "disabled");

				$("#step4Container")
						.append(
								"<div class=\"step\">Step 4/5 : Selection of the Criterias</div>");
				$("#step4Container").append(
						"<div class=\"data\" id=\"where\"></div>");
				$('#where')
						.append(
								'<a id="add_criterion_btn" href="#">Add Criterion</a><div id="list_of_criteria"></div>');
				// $("#where").append("<a id=\"addCrit\"
				// href=\"javascript:GSN.data.addCriteria(false)\">Add
				// criteria</a>");
				// $("#where").append('<br/><br/>');
				GSN.data.appendNextStepButton("where",
						"GSN.data.selectDataDisplay()");
													...
													function() {
														...
														if (field_name == 'timed') {
															...
															$(this)
																	.next()
																	.replaceWith(
																			default_min_criterion());
															$(this)
																	.next()
																	.next()
																	.replaceWith(
																			default_max_criterion());
														}
													});
								});
				function addCriteriaLine() {
					...
					for ( var i = 0; i < GSN.data.fields.length; i++) {
						newcrit += '<option value="' + GSN.data.fields[i]
								+ '">' + GSN.data.fields[i] + '</option>';
					}
					newcrit += '</select>';
					newcrit += '| Between ' + default_min_criterion();
					newcrit += ' and '
							+ default_max_criterion()
							+ '<a class="remove-criterion" href="#"> Remove</a></div>';
					return newcrit;
				}
		...
		// newcrit += "<select name=\"critJoin\" id=\"critJoin" + nb_crit + "\"
		// size=\"1\">";
		// var critJoin = new Array("AND", "OR");
		// for (var i=0; i < critJoin.length; i++) {
		// newcrit += "<option
		// value=\""+critJoin[i]+"\">"+critJoin[i]+"</option>";
		// }
		// newcrit += "</select>";
		// }
		...
		// newcrit += "<option value=\"" + neg[i] + "\" >" + neg[i] +
		// "</option>";
		// }
		// newcrit += "</select> ";
		// newcrit += "<select name=\"critfield\" id=\"critfield" + nb_crit +
		// "\" size=\"1\"
		// onChange=\"javascript:GSN.data.criteriaForType(this.value,"+nb_crit+")\">";
		// for (var i=0; i< GSN.data.fields.length; i++) {
		// newcrit += "<option value=\"" + GSN.data.fields[i] + "\">" +
		// GSN.data.fields[i] + "</option>";
		// }
		// newcrit += "</select> ";
		// var operators = new Array("&gt;", "&ge;", "&lt;", "&le;", "=",
		// "LIKE");
		// newcrit += "<select name=\"critop\" size=\"1\" id=\"critop" + nb_crit
		// + "\">";
		// for (i=0; i < operators.length; i++) {
		// newcrit += "<option value=\"" + operators[i] + "\" >" + operators[i]
		// + "</option>";
		// }
		// newcrit += "</select> ";
		// newcrit += "<input type=\"text\" name=\"critval\" id=\"critval" +
		// nb_crit + "\" size=\"18\">";
		// newcrit += " <a href=\"javascript:GSN.data.removeCrit("+nb_crit+")\"
		// id=\"remove" + nb_crit + "\"> (remove)</a>";
		...
		// if (field == "timed") {
		// $("#critval"+nb_crit).val(GSN.util.printDate((new
		// Date()).getTime()));
		...
		selectDataDisplay : function() {
			$(".nextStepButton").remove();
			$("#step4Container .data").hide("slow").prev().unbind('click')
					.click(function() {
						$(this).next().toggle("slow");
					});
			...
			$("#step5Container")
					.append(
							"<div class=\"step\">Step 5/5 : Selection of the Format</div>");
			$("#step5Container").append(
			...
			$("#display")
					.append(
							"<input type=\"radio\" id=\"dispChartAndData\" value=\"DispChartAndValue\" name=\"display\" onClick=\"javascript:$('.chartOption').show('slow');$('.cvsFormat').hide('slow');\" checked>Plot and View Data<br/>");
			$("#display")
					.append(
							"<input type=\"radio\" id=\"dispChart\" value=\"DispChart\" name=\"display\" onClick=\"javascript:$('.chartOption').show('slow');$('.cvsFormat').hide('slow');\">Plot Data Only<br/>");
			$("#display")
					.append(
							"<input type=\"radio\" id=\"dispData\" value=\"DispData\" name=\"display\" onClick=\"javascript:$('.chartOption').hide('slow');$('.cvsFormat').hide('slow');\">View Data Only<br/>");

			$("#display")
					.append(
							"<input type=\"radio\" id=\"popup\" value=\"popup\" name=\"display\" onClick=\"javascript:$('.chartOption').show('slow');$('.cvsFormat').hide('slow');\">In a new window<br/>");
			$("#display")
					.append(
							"<input type=\"radio\" id=\"CSV\" value=\"CSV\" name=\"display\" onClick=\"javascript:$('.chartOption').hide('slow');$('.cvsFormat').show('slow');\">Download data<br/>");
			$("#display").append('<br/>');

			$("#display").append(
					"<div class=\"chartOption\">Chart Selection:</div>");
			$("#display")
					.append(
							"<div class=\"chartOption\"><input type=\"radio\" id=\"barChart\" class=\"chartType\" name=\"chartType\" onClick=\"\" checked/>Bar Chart<br/></div>");
			$("#display")
					.append(
							"<div class=\"chartOption\"><input type=\"radio\" id=\"lineChart\" class=\"chartType\" name=\"chartType\" onClick=\"\"/>Line Chart<br/></div>");
			$("#display").append("<div class=\"chartOption\">Display:</div>");
			$("#display")
					.append(
							"<div class=\"chartOption\"><input type=\"radio\" id=\"allData\" class=\"dataDisplay\" name=\"dataDisplay\" onClick=\"\" checked/>All Values<br/></div>");
			$("#display")
					.append(
							"<div class=\"chartOption\"><input type=\"radio\" id=\"modData\" class=\"dataDisplay\" name=\"dataDisplay\" onClick=\"\"/>Snapshot Mode<br/></div>");

			// $("#display").append("<div
			// class=\"cvsFormat\">Delimiters:</div>");

			// $("#display").append("<div class=\"cvsFormat\"><input
			// type=\"radio\" id=\"semicolon\" class=\"chartType\"
			// name=\"delimiter\" value=\"semicolon\"
			// checked/>Semicolon<br/></div>");
			// $("#display").append("<div class=\"cvsFormat\"><input
			// type=\"radio\" id=\"tab\" class=\"chartType\" name=\"delimiter\"
			// value=\"tab\"/>Tab<br/></div>");
			// $("#display").append("<div class=\"cvsFormat\"><input
			// type=\"radio\" id=\"space\" class=\"chartType\"
			// name=\"delimiter\" value=\"space\"/>Space<br/></div>");
			// $("#display").append("<div class=\"cvsFormat\"><input
			// type=\"radio\" id=\"other\" class=\"chartType\"
			// name=\"delimiter\" value=\"other\"/>Other : <input type=\"text\"
			// name=\"otherdelimiter\" size=\"2\"/><br/></div>");

			$("#display")
					.append(
							"<div class=\"chartOption\"><input type=\"checkbox\" name=\"fullscreenView\" value=\"fullscreenView\" id=\"fullscreenView\"/>Go Fullscreen</div>");
			GSN.data.appendNextStepButton("display", "GSN.data.getDatas()",
					"getDatas", "Get Data");
			...
			$("#step5Container")
					.append(
							"<div id=\"warningmsg\">"
									+ "Warning: Not all the values are displayed. To get all the values, you have to download them."
									+ "</div>");
			$("#warningmsg").hide();
		},
		...
		getDatas : function(sql) {
			...
			if (!$("#CSV").attr("checked") || sql) {
				if (GSN.selectedSensors.length > 1)
					request = "multirequest=true";
				else
					request = "multirequest=false";
				...
				if ($("#someDatas").attr("checked")
						&& $("#nbOfDatas").attr("value") != "") {
					var nbValueToExtract = parseInt($("#nbOfDatas").attr(
							"value"));
					if (nbValueToExtract % 6 != 0) {
						nbValueToExtract = nbValueToExtract + 6
								- nbValueToExtract % 6;
					}
				...
				for ( var i = 0; i < GSN.data.criterias.length; i++) {
					if (i > 0) {
						request += "&critJoin="
								+ $("#critJoin" + GSN.data.criterias[i]).val();
					}
					request += "&neg="
							+ $("#neg" + GSN.data.criterias[i]).val();
					request += "&critfield="
							+ $("#critfield" + GSN.data.criterias[i]).val();
					request += "&critop="
							+ $("#critop" + GSN.data.criterias[i]).val();
					request += "&critval="
							+ $("#critval" + GSN.data.criterias[i]).val();
				}
				...
				if (sql) {
					...
					for ( var i = 0; i < GSN.selectedSensors.length; ++i) {
						...
						$.ajax({
							type : "GET",
							url : "/data?" + request + "&vsName="
									+ GSN.selectedSensors[i],
							success : function(msg) {
				 ...
				 * $("form").attr("target", "_blank"); $("form").attr("action",
				 * "/showData.jsp"); document.forms["formular"].submit(); }
				 */
				...
				$("#step2Container .data :enabled")
						.attr("disabled", "disabled");
				$("#step3Container .data :enabled")
						.attr("disabled", "disabled");
				$("#step4Container .data :enabled")
						.attr("disabled", "disabled");
		...
		displayDatas : function(request) {
			...
			if ($('#fullscreenView').attr("checked")) {
				...
				window.moveTo(0, 0);
				$('body').prepend(
						'<div id="fullscreenContent" style="width:'
								+ (screen.width) + ';"></div>');
				$('#fullscreenContent')
						.append(
								'<br/><input type="button" value="Back to Data Part" onclick="$(\'#container > div\').show(\'slow\'); $(\'#fullscreenContent\').remove();"/>')
				$("#fullscreenContent")
						.append(
								'&nbsp;&nbsp;&nbsp;<input type="button" value="Go to Table Values" onclick="document.location.href=\'#dataSet\'"/>');
				$('#fullscreenContent')
						.append(
								'<br/><br/><div id="seeLink">See chart:<br/><ul></ul></div>');
				$('#fullscreenContent')
						.append(
								'<span id="chartContainer"></span><br/><br/><br/><br/><br/><br/><br/><br/>');
			...
			for ( var i = 0; i < GSN.selectedSensors.length; ++i) {
				...
				$
						.ajax({
							...
							type : "GET",
							// TODO url:
							// "/data?"+request+"&vsName="+vsName+"&rand="+Math.random(),
							...
							success : function(answer) {
								...
								// Store the answer
								answerLinesFromXMLSensorNum[sensorNumber] = $(
										"line", answer);

								// If no answer return
								if ($("line", answer).size() == 0) {
									alert('No data corresponds to your request for the sensor: '
											+ vsName);
									return;
								}
								...
								$("table#dataSet", target).append(
										"<tr><td class='step' align='center' colspan='"
												+ nbFields + "'>"
												+ vsName.prettyString()
												+ "</td></tr>");
								...
								if ($("#nbOfDatas").attr("value"))
									nbOfData = parseInt($("#nbOfDatas").attr(
											"value")) + 1;
								else
									nbOfData = lines.size();
			...
			var nbSelectedFields = $("field", answerLinesFromXMLSensorNum[0]
					.get(0)).length;
			...
			for ( var m = 0; m < nbSelectedFields; m++) {
				if (regularExpression.test($("field",
						answerLinesFromXMLSensorNum[0].get(0)).eq(m).text())) {
					timedIndexInNbSelectedFieldsArray = m; // Will be useful to
					// detect if
					// parseFloat needed
					// or not for
					// comparison
			...
			var incModulo = 0;
			GSN.data.modulo = Math.floor(($("field",
					answerLinesFromXMLSensorNum[0]).length - 1)
					/ GSN.data.nbDispVal / nbSelectedFields);// Math.floor($("field",answerLinesFromXMLSensorNum[0]).length/GSN.data.nbDispVal/nbSelectedFields);
			...
			// creation of a Matrix dimension: nbSelectedSensors *
			// nbSelectedFields
			...
			for ( var i = 0; i < nbSelectedSensors; i++) {
				...
				for ( var m = 0; m < nbSelectedFields; m++) {
					...
					if (i == 0 && timedIndexInNbSelectedFieldsArray != m) {
						minValue[m] = parseFloat($("field",
								answerLinesFromXMLSensorNum[i].get(1)).eq(m)
								.text());
						maxValue[m] = parseFloat($("field",
								answerLinesFromXMLSensorNum[i].get(1)).eq(m)
								.text());
					...
					for ( var k = 0; k < (answerLinesFromXMLSensorNum[i].length - 1); k++) {
						...
						var field = $("field", answerLinesFromXMLSensorNum[i]
								.get(1 + k));

						if (timedIndexInNbSelectedFieldsArray != m)
							actualValue = parseFloat(field.eq(m).text());
						else
							actualValue = field.eq(m).text();

						GSN.log("Actual Value:" + actualValue);

						if (minValue[m] > actualValue)
							minValue[m] = actualValue;
						if (maxValue[m] < actualValue)
							maxValue[m] = actualValue;

						// Computation of the average
						if (k == 0 && timedIndexInNbSelectedFieldsArray != m)
							average = actualValue;
						else if (timedIndexInNbSelectedFieldsArray != m)
							average = (average + actualValue) / 2;
						else
							average = actualValue; // case it is the time

						// Construct values Array which contains the values for
						// the current selected sensor and for the current field
						nbValueToShow = parseInt($("#nbOfDatas").attr("value"));
						offset = 6 - nbValueToShow % 6;

						if ((answerLinesFromXMLSensorNum[i].length - 1) > 6
								&& $("#modData").attr("checked")) {
			...
			GSN.data.makeChart(nbSelectedFields,
					timedIndexInNbSelectedFieldsArray,
					answerLinesFromXMLSensorNum, values, minValue, maxValue,
					"barChart");
			...
			if ($('#barChart').attr("checked")
					&& ($('#dispChartAndData').attr("checked")
							|| $('#popup').attr("checked") || $('#dispChart')
							.attr("checked")))
				$('.lineChart').hide();
			if ($('#lineChart').attr("checked")
					&& ($('#dispChartAndData').attr("checked")
							|| $('#popup').attr("checked") || $('#dispChart')
							.attr("checked")))
				$('.barChart').hide();
			...
			if ($('#dispChart').attr("checked"))
				$('#dataSet').hide();

			$('#barChart').attr("onclick",
					"$('.lineChart').hide();$('.barChart').show();");
			$('#lineChart').attr("onclick",
					"$('.lineChart').show();$('.barChart').hide();");
		}// End displayDatas

		,
		modulo : 0,
		nbDispVal : NUMBER_OF_VALUE_TO_DISPLAY_IN_CHARTS_IN_MODULUS_MODE // Number
		// of
		// value
		// to
		// display
		...
		// mode
		,
		makeChart : function(nbSelectedFields,
				timedIndexInNbSelectedFieldsArray, answerLinesFromXMLSensorNum,
				values, minValue, maxValue, typeChart) {
			// Chart Part
			var nbValue = Math
					.floor($("field", answerLinesFromXMLSensorNum[0]).length
							/ nbSelectedFields) - 1;
			GSN.log("Nb Values extracted=" + nbValue);
			GSN.log("Nb Values requested=" + $("#nbOfDatas").val());
			GSN.log("Offset Values=" + (nbValue - $("#nbOfDatas").val()));

			if ($("#allDatas").attr("checked")
					&& $("#nbOfDatas").attr("value") < nbValue) {
				$("#warningmsg").show();
			} else {
				$("#warningmsg").hide();
			}
				...
				if (timedIndexInNbSelectedFieldsArray != m) {
					...
					if ($('#fullscreenView').attr("checked")) {
						$("#chartContainer").append(
								'<div class="' + typeChart + '" id="'
										+ typeChart + m + '"></div>');
						var so = new SWFObject(
								"./open-flash-chart/open-flash-chart.swf",
								typeChart + m, screen.width - 100,
								screen.height - 250, "9", "#FFFFFF");
						if ($("#" + typeChart).attr("checked")) {
							$("#chartContainer")
									.append(
											'&nbsp;&nbsp;&nbsp;<input type="button" value="Back to Data Part" onclick="$(\'#container > div\').show(\'slow\'); $(\'#fullscreenContent\').remove();"/>');
							$("#chartContainer")
									.append(
											'&nbsp;&nbsp;&nbsp;<input type="button" value="Go to Top" onclick="document.location.href=\'#fullscreenContent\'"/>');
							$("#chartContainer")
									.append(
											'&nbsp;&nbsp;&nbsp;<input type="button" value="Go to Table Values" onclick="document.location.href=\'#dataSet\'"/>');
						...
						$("#chartContainer").append(
								'<div style="border-width:10px;" class="'
										+ typeChart + '" id="' + typeChart + m
										+ '"></div><br/>');
						var so = new SWFObject(
								"./open-flash-chart/open-flash-chart.swf",
								typeChart + m, "450", "400", "9", "#FFFFFF");
					}
									...
									"Data viewer: "
											+ $(
													...
													answerLinesFromXMLSensorNum[0]
															.get(0)).eq(m)
													.text().prettyString()
											+ ",{font-size:20px; color: #000000; margin: 5px; padding:5px; padding-left: 20px; padding-right: 20px;}");
					...
					so.addVariable("x_label_style", "10,#000000,2," + x_step
							+ ",#eeeeee");
					...
					so.addVariable("y_legend", ""
							+ $("field", answerLinesFromXMLSensorNum[0].get(0))
									.eq(m).text().prettyString()
							+ ",12,#000000");
					...
					GSN.log($("field", answerLinesFromXMLSensorNum[0].get(0))
							.eq(m).text().prettyString()
							+ ":  min: "
							+ minValue[m]
							+ "   MAX: "
							+ maxValue[m]);
					// deltaDiv = Math.abs(Math.floor((parseFloat(maxValue[m]) -
					// parseFloat(minValue[m]))));
					// if(minValue[m] < 0) minVal =
					// Math.floor(1.05*parseFloat(minValue[m]));
		...
		makeLineChart : function(so, values, field) {
			for (i = 0; i < GSN.selectedSensors.length; ++i) {
				if (i == 0) {
					so.addVariable("line_dot", "2,"
							+ $('#hexcode-' + GSN.selectedSensors[i]).val()
							+ "," + GSN.selectedSensors[i] + ",10,4");
					so.addVariable("values", values[i][field]);
				} else {
					so.addVariable("line_dot_" + (i + 1), "2,"
							+ $('#hexcode-' + GSN.selectedSensors[i]).val()
							+ "," + GSN.selectedSensors[i] + ",10,4");
		...
		makeBarChart : function(so, values, field) {
			...
			for (i = 0; i < GSN.selectedSensors.length; ++i) {
				if (i == 0) {
					so.addVariable("bar_3d", "75,"
							+ $('#hexcode-' + GSN.selectedSensors[i]).val()
							+ "," + GSN.selectedSensors[0] + ",10");
					so.addVariable("values", values[i][field]);
				} else {
					so.addVariable("bar_3d_" + (i + 1), "75,"
							+ $('#hexcode-' + GSN.selectedSensors[i]).val()
							+ "," + GSN.selectedSensors[i] + ",10");
					so.addVariable("values_" + (i + 1), values[i][field]);
				}
			}
		}
	...
	util : {
		...
		printDate : function(date) {
			...
			var value = GSN.util.addleadingzero(date.getDate()) + "/"
					+ GSN.util.addleadingzero(date.getMonth() + 1) + "/"
					+ date.getFullYear();
			value += " " + GSN.util.addleadingzero(date.getHours()) + ":"
					+ GSN.util.addleadingzero(date.getMinutes()) + ":"
					+ GSN.util.addleadingzero(date.getSeconds());
			return value;
		}
		...
		/**
		 * Take an array return a sorted matrix with sensor name in first col
		 * and rubric name in the second col
		 */
			...
			for ( var i = 1; i < vsName.length; ++i) {
				for ( var similarDegree = 3; similarDegree < vsName[i].length; ++similarDegree) {
					regularExpression = new RegExp("^"
							+ vsName[i - 1].substr(0, similarDegree), "i");
					...
					if (regularExpression.test(vsName[i]) && !firstTimeTry) {
						vsNameRubric[i][1] = vsName[i - 1].substr(0,
								similarDegree);
						vsNameRubric[i - 1][1] = vsName[i - 1].substr(0,
								similarDegree);
					}
		...
		/**
		 * Take an array return a sorted array with grouping to the first
		 * underscore
		 */
		,
		regroupByUnderscore : function(vsName) {
			...
			for ( var i = 0; i < vsName.length; ++i) {
				...
				if (index >= 1)
					vsNameUnderscore[i][1] = name.substr(0, index);
				else
